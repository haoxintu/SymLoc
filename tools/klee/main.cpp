...
public:
  ...
  void incPathsExplored() { m_pathsExplored++;
    // new added for testing
      /*
      if (m_pathsExplored == 10000){
          printf("\nLimit of the number of paths is reached!\n");
          exit(1);
      }
      */
  ...
  if (!WriteNone) {
    ...
    if (success) {
      ...
      for (unsigned i=0; i<b.numObjects; i++) {
        ...
        //std::string target(o->name);
        //printf("target name = %s\n", o->name);
        //if (target.find("sym_") == std::string::npos){
    ...
    //printf("the number of malloc in this state = %d \n", state.mallocLocation.size());
    std::string file_name = getOutputFilename(getTestFilename("txt", id));
    if (file_name.size() == 0){
        klee_warning("open file failed of malloc functions");
        exit(1);
    }
    std::ofstream f(file_name);
    f << "name \t file_name \t src_file_line \t line \t address \t size \n";
    for (auto location : state.mallocLocation){
        std::string name = location.first;
        f << location.first << "\t";
        ...
        f<< location.second[5] << "\n";
        */
        //auto it = std::find_if(out.begin(), out.end(), [&name](std::vector<std::pair<std::string, std::vector<unsigned char>>>&e){return e.first == name;});
        /* TODO directly add address here
        //for (auto it : out){
            if (it.first == name){
                //std::string s(reinterpret_cast<char*>(it.second[0]));
                int numBytes = it.second.size();
                unsigned char *bytes = new unsigned char[numBytes];
                std::copy(it.second.begin(), it.second.end(), bytes);
                //std::string s((char*)bytes);
                //printf("bytes = %s\n", bytes);
    ...
    f.close();
    /*
    // Haoxin for add ktest for malloc address
    m_numTotalTests = 0;
    id = ++m_numTotalTests;
    if (success) {
      KTest b;
      ...
      if (!kTest_toFile(&b, getOutputFilename(getTestFilename("mktest", id)).c_str())) {
          klee_warning("unable to write output test case, losing it");
      } else {
        ++m_numGeneratedTests;
      }
