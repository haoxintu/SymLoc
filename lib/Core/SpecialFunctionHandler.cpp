...
void SpecialFunctionHandler::handleMalloc(ExecutionState &state,
  ...
  //int setSymMalloc = distr(gen); // random mode
  int setSymMalloc = 1;
...
void SpecialFunctionHandler::handleFree(ExecutionState &state,
  ...
  // *Haoxin start
    ...
    klee_warning_once(0, "execute symbolic free !");
    if (address->getKind() != 5){ // 5 is the Read Kind
        executor.terminateStateOnError(state, "an invalid free bug is detected!", Executor::User);
    }
    std::set<std::string> nameList;
    const Array *array = scan22(address, nameList);
...
void SpecialFunctionHandler::handleCheckMemoryAccess(ExecutionState &state,
  ...
  if (isa<ConstantExpr>(address)){
  // *Haoxin end
  address = executor.toUnique(state, arguments[0]);
  ...
  // *Haoxin start
    ...
    klee_warning("execute symbolic check memory access !");
  }
  //*Haoxin end
}
