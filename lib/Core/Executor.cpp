...
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal) {
  ...
  if (!isa<ConstantExpr>(condition)){ //a symbolic variable?
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Store: { //write operation
    ...
    if (!isa<ConstantExpr>(base)) {
        ...
        // now checking whether it is a use after free
        if (state.addressSpace.free_list.count(sym_name) != 0){
            llvm::errs() << "This symbolic is in the set!!!" << "\n";
            terminateStateOnExecError(state, "memory error: a use(write) after free is detected!");
        }
    }
  ...
  case Instruction::SExt: {
    CastInst *ci = cast<CastInst>(i);
    if (eval(ki, 0, state).value.get() != NULL){
...
void Executor::callExternalFunction(ExecutionState &state,
  ...
  //llvm::errs() << "Number of arguments : " << arguments.size() << "\n";
      ...
      *ai = optimizer.optimizeExpr(*ai, true);
      ...
      if (!isa<ConstantExpr>(arg)){
        ...
        for (auto iter = mmm1.begin(); iter != mmm1.end(); iter++){
            std::string key = iter->first;
            std::set<std::string>::iterator iter_set;
            iter_set = nameList1.find(key);
            if (iter_set != nameList1.end()){
                sym_name1 = key;
        }
        }
        llvm::errs() << "symbolic name in function call: " << sym_name1 << "\n";
        // now checking whether it is a use after free
        if (state.addressSpace.free_list.count(sym_name1) != 0){
            llvm::errs() << "This symbolic is in the set!!!" << sym_name1 << "\n";
            arg->dump();
            terminateStateOnExecError(state, "memory error: a use(read) after free is detected!");
        }
...
void Executor::executeAllocForMalloc(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
    ...
    // TODO skip here and write our own add or not add both are ok?
    state.addSymbolic(mo_buffer, array);
    ...
    state.addressSpace.location.push_back(std::to_string(target->info->line));
    state.addressSpace.location.push_back(std::to_string(mo_buffer->size));
    state.addressSpace.location.push_back(getAddressInfo(state, moMallocBufferAddress));
    // add stacktrace
    std::string MsgString;
    llvm::raw_string_ostream msg(MsgString);
    ...
    // THX code for add the range of symbolic addresses
    /*
    ref<Expr> e1 = symExpr;
    // add the first constraints address >= 0
    if (e1->getWidth() != Expr::Bool)
        e1 = UgeExpr::create(e1, ConstantExpr::create(0, e1->getWidth()));
    bool res1;
    bool success1 __attribute__ ((unused)) = solver->mustBeFalse(state, e1, res1);
    assert(success1 && "FIXME: Unhandled solver failure");
    addConstraint(state, e1);

    // add the first constraints address < 0x7fffffffffff
    ref<Expr> e2 = symExpr;
    if (e2->getWidth() != Expr::Bool)
        e2 = UltExpr::create(e2, ConstantExpr::create(0x7fffffffffff, e2->getWidth()));
    bool res2;
    bool success2 __attribute__ ((unused)) = solver->mustBeFalse(state, e2, res2);
  ...
  if (!isa<ConstantExpr>(address)){ //a symbolic variable?
    ...
    if (isSymbolicAddress && hasSymbolicMallocVariable(address, mmm)){ // handle symbolic address and aviod normal symbolic variable by comand
        ...
        if (hasSymbolicAddress && p_address.get() && symbolicAddress.get()) {
            ...
            unsigned long long actualAddressInt = tt->getZExtValue() + shared_map[sym_name].first->address;
...
void Executor::runFunctionAsMain(Function *f, //the initial execution function
				 int argc,// the number of arguments
				 char **argv, // the specific arguments
				 char **envp) { // the execution environments
  std::vector<ref<Expr> > arguments;
  ...
  ExecutionState *state = new ExecutionState(kmodule->functionMap[f]); // initialize state?

  if (pathWriter)
    state->pathOS = pathWriter->open(); // the path of concolic execution
  if (symPathWriter)
    state->symPathOS = symPathWriter->open(); // the path of symbolic execution
